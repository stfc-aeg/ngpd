...
int ngpd_spi_write_lmk04828_from_file(int path, int card, char *fname)
{
	...
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_spi_write_lmk04828_from_file: Invalid path=%d", path);
		return -1;
	}
	if (NGPDPath[path].generation == NGPDGenADQ14)
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_spi_write_lmk04828_from_file: Not supported on ADQ14");
		return -1;
	}
...
int ngpd_spi_write_lmk04828(int path, int card, int num, NGZMPSPIAddrData *reg)
{
	...
	if (path <0 || path >= NGPD_MAX_PATH || !NGPDPath[path].valid)
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_spi_write_lmk04828: Invalid path=%d", path);
		return -1;
	}
	if (NGPDPath[path].generation == NGPDGenADQ14)
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_spi_write_lmk04828: Not supported on ADQ14");
		return -1;
	}
	...
	if (card < 0)
	{
		...
		{
			rc = ngpd_spi_write_lmk04828(path, card, num, reg);
			if (rc < 0)
				break;
		}
		return rc;
	...
	for (i=0; i<num; i++)
	{
		// printf("Addr=%d=%04X, data=%02X\n", reg[i].addr, reg[i].addr, reg[i].data);
...
int ngpd_clock_setup_lmk04828(int path, int card, NGPDClockSetup *cset)
{
	...
	if (path <0 || path >= NGPD_MAX_PATH || !NGPDPath[path].valid)
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_clock_setup_lmk04828: Invalid path=%d", path);
		return -1;
	}
	if (NGPDPath[path].generation == NGPDGenADQ14)
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_clock_setup_lmk04828: Not supported on ADQ14");
		return -1;
	}
...
//		clock_regs[i].addr = LMK0482X_143_SYNC_MODE;
	...
	NGPDPath[path].clock_setup = *cset;
	return ngpd_spi_write_lmk04828(path, card, i, clock_regs);
}

int ngpd_clock_send_sysref(int path, int card, int num_pulse_code)
{
	u_int8_t cnt = num_pulse_code & 3;
	
	return ngzmp_spi_write_clk(path, card, LMK0482X_13E_SYSREF_PULSE_CNT, 1, &cnt);
}
...
int ngpd_clock_wait_locked(int path, int card)
{
	...
	if (path <0 || path >= NGPD_MAX_PATH || !NGPDPath[path].valid)
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_clock_wait_locked: Invalid path=%d", path);
		return -1;
	}
	if (NGPDPath[path].generation == NGPDGenADQ14)
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_clock_wait_locked: Not supported on ADQ14");
		return -1;
	}
	if (NGPDPath[path].dummy_system & NGPDDummy_FPGA)
	{
		printf("Dummy:ngpd_clock_wait_locked(card=%d)\n", card);
		return 0;
	}
	if (card < 0)
	{
	...
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_clock_wait_locked: card=%d out of range 0...%d", card, NGPDPath[path].num_cards-1);
		return -1;
	}
	else
	{
	...
	for (i=0;i<100; i++)
	{
		...
		for (card=first_card; card <=last_card; card++)
		{
			...
			if (rc < 0)
			{
				...
				strncpy(old_msg, ngpd_error_message, NGPD_MAX_ERROR_MESSAGE);
				old_msg[NGPD_MAX_ERROR_MESSAGE] = 0;
				snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_clock_setup: reading LMK04828 status : %s", old_msg);
				return rc;
			}
	...
	for (card=first_card; card <=last_card; card++)
	{
		...
		if (rc < 0)
		{
			...
			strncpy(old_msg, ngpd_error_message, NGPD_MAX_ERROR_MESSAGE);
			old_msg[NGPD_MAX_ERROR_MESSAGE] = 0;
			snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_clock_setup: Writing LMK04828 status to clear LOST  : %s", old_msg);
			return rc;
		}
	}
	return 0;
}
/* Read status from LMK04828 to check that clock is still locked and that lock was not lost since last poll */
int ngpd_clock_check_locked(int path, int card)
{
	...
	if (path <0 || path >= NGPD_MAX_PATH || !NGPDPath[path].valid)
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_clock_check_locked: Invalid path=%d", path);
		return -1;
	}
	if (NGPDPath[path].generation == NGPDGenADQ14)
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_clock_check_locked: Not supported on ADQ14");
		return -1;
	}
	...
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_clock_check_locked: card=%d out of range 0...%d", card, NGPDPath[path].num_cards-1);
		return -1;
	}
	else
	{
	...
	for (card=first_card; card <=last_card; card++)
	{
		...
		if (!(status[0] & LMK0482X_RB_PLLX_LD) || !(status[1] & LMK0482X_RB_PLLX_LD) || (status[0] & LMK0482X_RB_PLLX_LD_LOST) || (status[1] & LMK0482X_RB_PLLX_LD_LOST) )
		{
			...
			if (rc < 0)
			{
				...
				strncpy(old_msg, ngpd_error_message, NGPD_MAX_ERROR_MESSAGE);
				old_msg[NGPD_MAX_ERROR_MESSAGE] = 0;
				snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_clock_check_locked: Writing LMK04828 staus to clear LOST  : %s", old_msg);
				return rc;
			}
		}
	}
...
{
	int first_card, last_card, c;
	int rc;
	u_int32_t clock_select;
	
	if (path <0 || path >= NGPD_MAX_PATH || !NGPDPath[path].valid)
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_clock_setup: Invalid path=%d", path);
		return -1;
	}
	if (NGPDPath[path].generation == NGPDGenADQ14)
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_clock_setup: Not supported on ADQ14");
		return -1;
	}
	...
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_clock_setup: card=%d out of range 0...%d", card, NGPDPath[path].num_cards-1);
		return -1;
	}
	else
	{
	...
	{
		/* For master/slave arrangement, detect master cared here */
		...
		rc = ngpd_write_glob_regs(path, c, NGZMP_CLK_SELECT, 1, &clock_select);
		if (rc < 0)
		{
			char old_msg[NGPD_MAX_ERROR_MESSAGE+2];
			strncpy(old_msg, ngpd_error_message, NGPD_MAX_ERROR_MESSAGE);
			old_msg[NGPD_MAX_ERROR_MESSAGE] = 0;
			snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_clock_setup: Writing clock control register : %s", old_msg);
			return rc;
		}
		...
		if ((rc=ngpd_clock_setup_lmk04828(path, c, cset)) < 0)
			return rc;
		
		if ((rc=ngpd_clock_wait_locked(path, card)) < 0)  // 16/2/2023 Updated to check clock is locked before switching FPGA to this clock
			return rc;
