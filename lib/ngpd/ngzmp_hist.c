...
static int ngzmp_hist_read_dummy(int path, int chan, u_int32_t offset, u_int32_t size, u_int32_t *data);
...
@param num_words	Number of AXI word to clear within stream. Setting start=0, num=0 causes full memory to be cleared.
@return   			0 on success or -1 on error
*/
...
int ngzmp_hist_read_chan(int path, int chan, u_int32_t offset, u_int32_t size, u_int32_t *data)
{
	...
	if (NGPDPath[path].dummy_system & NGPDDummy_FPGA)
		return ngzmp_hist_read_dummy(path, chan, offset, size, data);
...
/**
 * @ingroup top_level
 * Enable the histogram data path. Usually set to the normal run mod, but can take data from a test pattern path.
 *
 * @param path a handle to the top level of the ngpd system returned from {@link ngpd_config_ngzmp()}.
 * @param card 	Card index within system 0 .. num_cards-1
 * @param enable	Enable options as hist_conf	Pointer to histogram structure.
 * @return {@link NGPD_OK} or a negative error code.
 */
int ngzmp_hist_enable(int path, int card, NGZMPHistEnable enable)
{
	u_int32_t hist_gpio_cont = enable;

	if (path <0 || path >= NGPD_MAX_PATH || !NGPDPath[path].valid)
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngzmp_hist_enable: Invalid path=%d", path);
		return -1;
	}

	if (NGPDPath[path].generation == NGPDGenADQ14)
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngzmp_hist_enable: Not supported on ADQ14");
		return -1;
	}
	return ngzmp_hist_write_reg(path, -1, NGZMP_HIST_GPIO_OFFSET, 1, &hist_gpio_cont);
}

static int ngzmp_hist_read_dummy(int path, int chan, u_int32_t offset, u_int32_t size, u_int32_t *data)
{
	NGZMPHistConf hist_conf;
	int rc;
	int nbins_hgt, nbins_tail_sum;
	int num_ngp;
	int i, hgt, tsum;
	double n_hgt, n_tsum, z;
	double two_ss_n, two_ss_ts;
	double g_mean, g, two_ss_g;
	if ((rc = ngzmp_hist_read_chan_config(path, chan, &hist_conf)) < 0)
		return rc;


	nbins_hgt = 1 << hist_conf.nbits_height;
	nbins_tail_sum = 1 << hist_conf.nbits_tail_sum;
	num_ngp = (hist_conf.separate_ngp)?3:1;
	
	if (offset+size > nbins_hgt*nbins_tail_sum*num_ngp)
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngzmp_hist_read_dummy:  offset %u for %u does not fit in histogram size %u", offset, size, nbins_hgt*nbins_tail_sum*num_ngp);
		return -1;
	}
/*
	hist_conf.enb_hgt_sum;
	hist_conf.enb_hgt_fall;
	hist_conf.discard_pu;
	hist_conf.shift_height;
	hist_conf.shift_tail_sum;
	hist_conf.nbits_fall_time;
	hist_conf.shift_fall_time;
*/
	n_hgt = 0.3*nbins_hgt+0.05	* (chan % 8); 
	n_tsum = 0.2*nbins_tail_sum;
	
	
	two_ss_n = 2.0*0.0025*nbins_hgt*nbins_hgt;
	two_ss_ts = 2.0*0.001*nbins_tail_sum*nbins_tail_sum;
	two_ss_g = 2.0*0.0005*nbins_tail_sum*nbins_tail_sum;

	for (i=0; i<size; i++)
	{
		hgt = (offset+i) % nbins_hgt;
		tsum = ((offset+i)/ nbins_hgt)%nbins_tail_sum;
		z = 1000.0*(1.0+0.1*chan);
		if (nbins_hgt > 1)
			z *= exp(-(hgt-n_hgt)*(hgt-n_hgt)/two_ss_n);
		if (nbins_tail_sum > 1)
			z *= exp(-(tsum-n_tsum)*(tsum-n_tsum)/two_ss_ts);
		g_mean = nbins_tail_sum*(0.2*hgt)/(double)nbins_hgt;
		g = 300*(1-0.05*chan);
		if (nbins_tail_sum > 1)
			g *= exp(-(tsum-g_mean)*(tsum-g_mean)/two_ss_g);
		*data++ = (int)(z+g);
	}
	return 0;
}
