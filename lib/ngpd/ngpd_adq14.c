...
@defgroup toplevel    	Top level functions.
						Functions that form the preferred API for user code.
@defgroup fullcontrol 	Full control functions.
						API that allows full control of NGPD algorithm. 
...
@defgroup debug       	Debugging functions.
						Functions provided to assist in system debugging.
@defgroup specialist  	Specialist API.
						API that allows unconventional modes of operation.
@defgroup scope_module  Scope Mode API.
						Functions provided to allow access to the scope mode data.
@defgroup calibration	Automatic calibrations functions in libcal.a
*/
...
void ngpd_close(int path)
{
	...
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_close: Invalid path=%d", path);
		return ;
	}
