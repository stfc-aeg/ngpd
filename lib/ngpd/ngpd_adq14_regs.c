...
int ngpd_write_chan_cont(int path, int chan, NGPDChanCont *cc)
{
	...
	if (cc->use_pb_start)
		chan_cont |= NGZMP_CC_SRC_USE_PB_START;
	chan_cont |= NGZMP_CC_SRC_SET(cc->data_src);
	if (cc->inv_data)
		chan_cont |= NGZMP_CC_INV_DATA;
	return ngpd_write_chan_regs(path, chan, NGZMP_REGION_REGS, NGPD_CHAN_CONT, 1, &chan_cont);
}

int ngpd_write_chan_cont_reg(int path, int chan, u_int32_t chan_cont)
{
	if (path <0 || path >= NGPD_MAX_PATH || !NGPDPath[path].valid)
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_write_chan_cont_reg: Invalid path=%d", path);
		return -1;
	}
	if (NGPDPath[path].generation == NGPDGenADQ14)
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_write_chan_cont_reg: Not supported on ADC14");
		return -1;
	}
	return ngpd_write_chan_regs(path, chan, NGZMP_REGION_REGS, NGPD_CHAN_CONT, 1, &chan_cont);
}

int ngpd_read_chan_cont(int path, int chan, NGPDChanCont * cc)
{
	u_int32_t chan_cont;
	int rc;
	if (path <0 || path >= NGPD_MAX_PATH || !NGPDPath[path].valid)
	...
	rc = ngpd_read_chan_regs(path, chan, NGZMP_REGION_REGS, NGPD_CHAN_CONT, 1, &chan_cont);
	memset(cc, 0, sizeof(NGPDChanCont));
	if (chan_cont & NGZMP_CC_SRC_USE_PB_START)
		cc->use_pb_start = 1;
	cc->data_src = NGZMP_CC_SRC_GET(chan_cont);
	if (chan_cont & NGZMP_CC_INV_DATA)
		cc->inv_data =1;
	return rc;
}
...
/*			for (i=0; i<256; i++)
				printf("ngpd_write_measure chan=%d, tail_thres[%d] m, c=(%d, %d)\n", chan, i, measure->tail_thres_m[i], measure->tail_thres_c[i]);
*/
...
/* Read scope status AND cycles register.
...
int ngpd_read_scope_status(int path, int card, u_int32_t *scope_status)
{
	...
	if (NGPDPath[path].generation == NGPDGenADQ14)
		return ngpd_read_glob_regs(path, card, NGPD_GLOB_SCOPE_STATUS, 2, scope_status);
	else
		return ngpd_read_glob_regs(path, card, NGZMP_SCOPE_STATUS, 2, scope_status);
}

/* Read scope status, cycles and for ngzmp read ITFG remaining time register. Could be back ported to ADQ14 version.

 */
int ngpd_read_itfg_status(int path, int card, NGPDITFGStatus *itfg_status)
{
	u_int32_t status_regs[4];
	int rc;
	if (path < 0 || path >= NGPD_MAX_PATH || !NGPDPath[path].valid)
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_read_itfg_status: Invalid Path");
		return NGPD_ERROR;
	}
	memset(status_regs, 0, sizeof(status_regs));
	if (NGPDPath[path].generation == NGPDGenADQ14)
	{
		rc = ngpd_read_glob_regs(path, card, NGPD_GLOB_SCOPE_STATUS, 2, status_regs);
		itfg_status->flags = status_regs[0];
		itfg_status->cycles = status_regs[1];
		itfg_status->timer_us = 0;
	}
	else
	{
		rc = ngpd_read_glob_regs(path, card, NGZMP_SCOPE_STATUS, 4, status_regs);
		itfg_status->flags = status_regs[0];
		itfg_status->cycles = status_regs[1];
		itfg_status->timer_us = status_regs[3];
	}
	return rc;
}
...
int ngpd_read_revision(int path, int card, u_int32_t *rev)
{
	if (path < 0 || path >= NGPD_MAX_PATH || !NGPDPath[path].valid)
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_read_revision: Invalid Path");
		return NGPD_ERROR;
	}
	
	if (NGPDPath[path].generation == NGPDGenADQ14)
		return ngpd_read_glob_regs(path, card, NGPD_GLOB_REVISION, 1, rev);
	else
		return ngpd_read_glob_regs(path, card, NGZMP_REVISION, 1, rev);
}