...
int ngzmp_do_config(int ncards, char* baseIPaddress, int basePort, char* baseMACaddress, int num_chan,
	...
	for (i = 0; i < ncards; i++) {
		...
		if (debug) {
			printf("ngzmp_do_config: Card %d\n", i);
			printf("ngzmp_do_config: host10GigeIPaddress %s\n", host10GigeIPaddress[i]);
			printf("ngzmp_do_config: zynqmpIPaddress %s\n", zynqmpIPaddress[i]);
			printf("ngzmp_do_config: zynqmp10GigeIPaddress %s\n", zynqmp10GigeIPaddress[i]);
			printf("ngzmp_do_config: zynqmpMACaddress %s\n", zynqmpMACaddress[i]);
		}
	}
	sprintf(defaultScopeModname, "ngpd_scope%d", starting_card);
	if (debug) {
		printf("ngzmp_do_config: 1GigePort %d\n", tcpPort);
		printf("ngzmp_do_config: 10GigePort %d\n", udpPort);
		printf("ngzmp_do_config: scope module name %s\n", defaultScopeModname);
	}
...
int ngpd_config_tcp(char zynqmpHostName[][NGPD_MAX_IP_CHARS], int zynqmpPort, int ncards, int num_chan, int debug, NGPDDummy dummy_system) {
	...
	if (ncards > 1)
	{
		...
		NGPDPath[path].generation = NGPDPath[NGPDPath[path].sub_path[0]].generation;
	...
	if ((dummy_system & NGPDDummy_FPGA) == 0)
	{
		...
		if (zynqmpHandle == NULL) {
			...
			snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_config: Cannot initialise Fem %s on port %d: %s", zynqmpHostName[0],
					zynqmpPort, zynqmp_get_error_message());
			return NGPD_ERROR;
		}
...
int ngpd_read_config_buffer(int path, int card, int offset, int num_bytes, u_int8_t *data)
{
	if (path <0 || path >= NGPD_MAX_PATH || !NGPDPath[path].valid)
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_read_config_buffer: Invalid path=%d", path);
		return -1;
	}
	if (NGPDPath[path].generation == NGPDGenADQ14)
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_read_config_buffer: Not supported on ADQ 14");
		return -1;
	}
	else
	{
		...
		if (card < 0 || card >= NGPDPath[path].num_cards)
		{
			snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_read_config_buffer: Card=%d out of range 0...%d", card, NGPDPath[path].num_cards-1);
			return -1;
		}
		if (NGPDPath[path].type == NGPDComposite)
		{
			...
			if (this_path <0)
			{
				snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_read_config_buffer: Card=%d resolved illegal sub_path=%d", card, this_path);
				return -1;
			}
		}
		else
		...
		{
			snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_read_config_buffer: Not supported on dummy system");
			return -1;
		}
		if (offset < 0 || offset >= ZYNQMP_CONFIG_BUFFER_SIZE || num_bytes < 1 || offset+num_bytes > ZYNQMP_CONFIG_BUFFER_SIZE)
		{
			snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_read_config_buffer: offset=%d + num=%d outside buffer size %d", offset, num_bytes, ZYNQMP_CONFIG_BUFFER_SIZE);
			return -1;
		}	
		return zynqmp_access_read(NGPDPath[this_path].zynqmp, ZYNQMP_BUS_CONFIG, ZYNQMP_WIDTH_BYTE, offset, num_bytes, 1, (u_int8_t *)data);
	}
}
...
int ngpd_write_config_buffer(int path, int card, int offset, int num_bytes, u_int8_t *data)
{
	...
	if (path <0 || path >= NGPD_MAX_PATH || !NGPDPath[path].valid)
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_write_config_buffer: Invalid path=%d", path);
		return -1;
	}
	if (NGPDPath[path].generation == NGPDGenADQ14)
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_write_config_buffer: Not supported on ADQ 14");
		return -1;
	}
	if (card < 0)
	{
		for (card=0; card< NGPDPath[path].num_cards; card++)
		{
			rc = ngpd_write_config_buffer(path, card, offset, num_bytes, data);
			if (rc < 0)
				return rc;
		}
		return 0;
	...
	if (card >= NGPDPath[path].num_cards)
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_write_config_buffer: Card=%d out of range 0...%d", card, NGPDPath[path].num_cards-1);
		return -1;
	}
	if (NGPDPath[path].type == NGPDComposite)
	{
		...
		if (this_path <0)
		{
			snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_write_config_buffer: Card=%d resolved illegal sub_path=%d", card, this_path);
			return -1;
		}
	}
	else
	...
	if (NGPDPath[path].dummy_system & NGPDDummy_FPGA)
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_write_config_buffer: Not supported on dummy system");
		return -1;
	}
	if (offset < 0 || offset >= ZYNQMP_CONFIG_BUFFER_SIZE || num_bytes < 1 || offset+num_bytes > ZYNQMP_CONFIG_BUFFER_SIZE)
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_write_config_buffer: offset=%d + num=%d outside buffer size %d", offset, num_bytes, ZYNQMP_CONFIG_BUFFER_SIZE);
		return -1;
	}
		
	return zynqmp_access_write(NGPDPath[this_path].zynqmp, ZYNQMP_BUS_CONFIG, ZYNQMP_WIDTH_BYTE, offset, num_bytes, 1, (u_int8_t *)data);
}
