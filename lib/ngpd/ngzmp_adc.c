...
// AD9694_SPI_PAIR_PIN_CONT1		0x0040	// Can output Local Multi Frame Clock LMFC if requried for debug 
	...
	{AD9694_SPI_PAIR_JESD_LINK_JTX_OFFSET, 0xA,	NGZMPADC_Pair },	// 0x0578 .. Move LMFC start later in TX to match delays wit extra register entering Zynq?
...
int ngzmp_adc_setup_adc(int path, int card, NGZMPADCFlags flags)
{
	...
	for (link=0; link<4; link++)
	{
		...
		if (rc < 0)
			goto exit_error;
		reg32 = 0;
/*		rc = ngpd_write_jesd_regs(path, card, link, JESD_RESET, 1, &reg32); // This is self clearing in the JESD204 core, but not in the JESD204C core.
		if (rc < 0)
			goto exit_error; */
	}
	...
	for (link=0; link<4; link++)
	{
		...
		do
		{
			rc = ngpd_read_jesd_regs(path, card, link, JESD_RESET, 1, &reg32);
			if (rc < 0)
				goto exit_error;
		} while ((reg32&1) && retries++<1000); // Running locally on ZynqMP takes abut 297 retries, will be much slower via TCP from x86_64 
		if (reg32&1)
		{
			snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngzmp_adc_setup_adc: link %d still in reset after 1000 reads\n", link);
			rc = -1;
			break;
		}
		printf("ngzmp_adc_setup_adc: Link %d, retries to complete reset=%d\n", link, retries);
	}
