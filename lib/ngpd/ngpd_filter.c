...
static int ngpd_save_filter_type(int path, int chan, NGPDFilter *filter)
{
	...
	{
		if ((rc=ngpd_resolve_path_chan_card(path, chan, &this_path, &chan_of_card, &card)) < 0)
			return rc;
		NGPDPath[this_path].filter[chan_of_card] = *filter;
		if (NGPDPath[path].generation != NGPDGenADQ14 && !(NGPDPath[path].dummy_system & NGPDDummy_FPGA))
		{
			int offset = sizeof(NGPDFilter)*chan_of_card;
			rc = ngpd_write_config_buffer(path, card, offset, sizeof(NGPDFilter), (u_int8_t*)filter);
		}
	}
	return rc;
}
int ngpd_get_filter_type(int path, int chan, NGPDFilter *filter)
{
	...
	{
		int offset = sizeof(NGPDFilter)*chan_of_card;
		rc = ngpd_read_config_buffer(path, card, offset, sizeof(NGPDFilter), (u_int8_t*) &(NGPDPath[this_path].filter[chan_of_card]));
	}
...
int ngpd_filter_load_integer(int path, int chan, int num_words, u_int32_t *data, u_int32_t *statusP)
{
	...
	int i, attempt;
	int card=0;
	NGPDFilter filter = {NGPDFilter_Custom, 0, 0, 0.0};
	...
	if (NGPDPath[path].generation == NGPDGenADQ14)
	{
		...
		for (attempt=0; attempt<10; attempt++)
		{
			...
			if (!(status & NGPD_FILTER_STATUS_RELOAD_TREADY))
			{
				...
				status |= NGPD_FILTER_STATUS_MISSING_TREADY;
				coeff = NGPD_FILTER_FORCE_CONF;
				ngpd_write_chan_regs(path, chan, NGPD_REGION_REGS, NGPD_CHAN_FILTER, 1, &coeff);
				if (NGPDPath[path].debug > 0)
					printf("ngpd_filter_load_integer: Attempt %d< Sending force Config to unlock missing tready\n", attempt);
				continue;
...
int ngpd_filter_read(int path, int chan, int num_words, u_int32_t *data)
{
	...
	if (path <0 || path >= NGPD_MAX_PATH || !NGPDPath[path].valid)
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_filter_read: Invalid path=%d", path);
		return -1;
	}
	if (num_words  != NGPD_FILT_NUM_COEFF)
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_filter_read:expected number of coefficients=%d, not %d", NGPD_FILT_NUM_COEFF, num_words);
		return -1;
	}
	if (chan < 0 || chan >= NGPDPath[path].num_chan)
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_filter_read: chan %d out of range 0..%d", chan, NGPDPath[path].num_chan-1);
		return -1;
	}

	if (NGPDPath[path].generation == NGPDGenADQ14)
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_filter_read: not supported on ADQ14");
		return -1;
	}
	else
	{
		...
		if ((rc=ngpd_resolve_path_chan_card(path, chan, &this_path, &chan_of_card, NULL)) < 0)
		{
			char old_msg[NGPD_MAX_ERROR_MESSAGE+2];
			strcpy(old_msg, ngpd_error_message);
			snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_filter_read: Invalid channel : %s", old_msg);
			ngpd_error_message[NGPD_MAX_ERROR_MESSAGE]=0;
			return rc;
		}
		if (NGPDPath[path].dummy_system & NGPDDummy_FPGA)
		{
			printf("Dummy: ngpd_filter_read(chan=%d, chan_of_card=%d)\n", chan, chan_of_card);
			return 0;
		}
		rc = zynqmp_personality_read(NGPDPath[this_path].zynqmp, ZYNQMP_DMA_CMD_LOAD_FILTER, ZYNQMP_WIDTH_LONG, chan_of_card, num_words, 1, (u_int8_t *)data, 0, 0, 0);
		if (rc < 0)
		{
			snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_filter_read: zynqmp_personality_read fails: %s", zynqmp_get_error_message());
			ngpd_error_message[NGPD_MAX_ERROR_MESSAGE]=0;
			return rc;
		}
...
int ngpd_filter_load_trapezoid(int path, int chan, int wid_top, int wid_bot)
{
	...
	if (path <0 || path >= NGPD_MAX_PATH || !NGPDPath[path].valid)
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_filter_load_trapezoid: Invalid path=%d", path);
		return -1;
	}

	if (wid_bot <= wid_top)
	{
		snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_filter_load_trapezoid: require wid_top=%d to be less than wid_bot=%d", wid_top, wid_bot);
		return -1;
	}
