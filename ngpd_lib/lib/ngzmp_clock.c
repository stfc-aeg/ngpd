...
int ngpd_clock_wait_locked(int path, int card)
{
	...
	for (card=first_card; card <=last_card; card++)
	{
		...
		if (rc < 0)
		{
			...
			strncpy(old_msg, ngpd_error_message, NGPD_MAX_ERROR_MESSAGE);
			old_msg[NGPD_MAX_ERROR_MESSAGE] = 0;
			snprintf(ngpd_error_message, NGPD_MAX_ERROR_MESSAGE, "ngpd_clock_setup: Writing LMK04828 staus to clear LOST  : %s", old_msg);
			return rc;
		}
	}
	return 0;
...
int ngpd_clock_setup(int path, int card, NGPDClockSetup *cset)
{
	...
	{
		/* For master/slave arangement, detect master cared here */
