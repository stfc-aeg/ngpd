...
int dma_upload_short(int path)
{
	...
	rc = ngpd_dma_upload(path, card, 0, NUM_WORDS_SHORT, wr_buff);
...
int dma_upload_medium(int path)
{
	...
	rc = ngpd_dma_upload(path, card, 0, NUM_WORDS_MEDIUM, wr_buff);
...
int dma_upload_full(int path)
{
	...
	rc = ngpd_dma_upload(path, card, 0, NUM_WORDS_LONG, wr_buff);
...
int dma_run_playback(int path, int card)
{
	u_int32_t pb_skip=8;

	dma_upload_medium(path);

	ngpd_write_glob_regs(path, card, NGPD_PLAYBACK_SKIP, 1, &pb_skip);
...
int fill_scope_mod(NGPDScopeModule *mod)
{
	...
	inc = ngpd_scope_mod_get_inc(mod, 0);
